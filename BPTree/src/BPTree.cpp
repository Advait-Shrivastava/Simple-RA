...
void BPTree::delete_key(const Key &key)
{
    ...
    TreePtr leftSib = NULL_PTR;
    TreePtr rightSib = NULL_PTR;
    TreePtr ptrToParent = NULL_PTR;


    root_node->delete_key(key, leftSib, rightSib, ptrToParent, DELETE_MARKER);
    // height of bptree should be decreased

    root_node = TreeNode::tree_node_factory(this->root_ptr);
    ...
    root_node = TreeNode::tree_node_factory(this->root_ptr);
    if(root_node->node_type == INTERNAL){
        // cout<<"Going for delete check"<<endl;
        InternalNode* rootCheck = new InternalNode(this->root_ptr);
        rootCheck->delete_key_checks(key);
        delete rootCheck;
    }
